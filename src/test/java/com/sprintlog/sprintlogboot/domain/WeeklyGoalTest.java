...
public class WeeklyGoalTest {
  ...
  class AchievementRate{
    ...
    void 절반이면_50퍼센트() {
      WeeklyGoal goal=new WeeklyGoal(60); // 60: 목표한 시간 의미
  ...
  class Achieved {
    ...
    void 목표_이상이면_달성() {
      ...
      WeeklyGoal goal = new WeeklyGoal(60);
      assertThat(goal.isAchieved(60)).isTrue(); // 목표 시간, 공부 시간 전달해 true/false
  ...
  // 두 번재 TDD 사이클 - 목표 시간 대비 공부시간 계산해서 남은 시간이 얼마인지를 알려주는 remainingMinutes
  @Nested
  @DisplayName("남은 시간")
  class Remaining {
