...
// SpringBootTest 사용 시 @ActiveProfiles 사용 권장
@SpringBootTest // Spring의 모든 빈을 등록해 컨테이너에 세팅, 모키토가 아닌 진짜 객체 사용한다는 의미
@ActiveProfiles("test") // test yaml에 있는 내용 그대로 가져올 수 있다, test 실행 시 프로필 active를 test로 -> test.yml 활성화시키겠다
...
public class ActivityServiceIntegrationTest {
  ...
  void 성공하면_둘_다_커밋() {
    ...
    // 이전에 셌던 count 보다 하나씩 데이터 많아야 한다
  ...
  void 전파_활동은_롤백_시도이력은_남음() {
    ...
    // 별도의 트랜잭션으로 동작되었기 때문에 제대로 insert되어야 한다
  }
}
