...
public class ActivityE2ETest {
  ...
  // 평소에는 포트번호가 고정되어 있지만 현재 테스트는 랜덤으로 보내기 때문
  @LocalServerPort int port; // 서버가 실제로 배정받은 랜덤 포트 번호를 이 필드에 주입
  @Autowired
  TestRestTemplate rest; // 진짜 HTTP 요청을 보내는 클라이언트를 주입받는다
  ...
  // build 폴더 아래에 폴더를 세팅하면 gitignore 대상이고, gradle clean할 때 알아서 지워준다
  ...
  void clean(){
    ...
    // 매 테스트마다 업로드 폴더를 비운다, 앞의 테스트가 뒤의 테스트에 영향을 주면 안 되기 때문
  ...
  // 문자열 상수 이용해 파일 선언해준 것, Base64 바이트 코드 - 이미지 역할 대신, 1 픽셀 x 1 픽셀 실제 이미지 파일

  private Resource fileResource(String filename, byte[] bytes) {
    return new ByteArrayResource(bytes) {
      @Override
      public String getFilename() {
        return filename;   // 이게 없으면 스프링 내에서 실제 파일 파트로 인식되지 않는다
      }
    };
  }
  ...
  // 생성 요청을 멀티파트로 보내주는 헬퍼 메서드
  // JSON 문자열을 넘기면 알아서 멀티파트로 포장해준다
  private ResponseEntity<String> multipartCreate(String dataJson) {
    ...
    // 실제로 생성 요청 보낼 때 JSON 담아 보낸다
    // 헤더 정보를 세팅 (요청 컨텐트 타입이 JSON)

    HttpEntity<String> dataPart = new HttpEntity<>(dataJson, dataHeaders); // body(dataJson)를 먼저 써줘야 한다
    // 헤더와 바디를 하나의 객체로 감싸준다

    // 일반 Map은 하나의 키에 하나의 값만 넣을 수 있다
    // 키는 중복이 발생할 수 없다
    // MultiValueMap은 하나의 키에 여러 개의 값을 리스트로 매핑해 준다 'Map<Key, List<Value>>'
    // TestRestTemplate이 multipart/form-data 본문을 생성할 때 MultiValueMap을 받도록 설계되어 있다
    MultiValueMap<String, Object> parts = new LinkedMultiValueMap<>();
    parts.add("data", dataPart);

    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.MULTIPART_FORM_DATA); // 전체 요청은 multipart/form-data 요청
  ...
  void 생성하고_다시_조회() {
    ...
    assertThat(created.getStatusCode()).isEqualTo(HttpStatus.CREATED); // 201 응답이 왔을 것
    URI location = created.getHeaders().getLocation();
    assertThat(location).isNotNull(); // Location 헤더가 있을 것


    // (2) 그 주소로 다시 GET -> 방금 저장한 게 진짜 DB에서 나올 것
    // 전 계층을 왕복해볼 것
  ...
  void 파일과_함께_생성() throws IOException {
    // (1) 진짜 HTTP POST -> 컨트롤러 -> 서비스 -> 진짜 DB에 저장
    byte[] sent = Base64.getDecoder().decode(PNG_1X1); // 문자열-진짜 파일로 인식하기 위해

    ResponseEntity<String> created = multipartCreate("""
                {"category":"LECTURE","title":"E2E 강의","minutes":45,
                 "visibility":"PUBLIC","instructorName":"이강사","studiedOn":"2026-01-01"}
                """, png("proof.png"), MediaType.IMAGE_PNG); // 활동을 생성할 때 만들 json
    ...
    assertThat(saved.get(0)).matches("[0-9a-f]{32}\\.png"); // 정규표현식으로 UUID 파일명이 맞는지 확인, UUID 총 32글자

    // 업로드 된 파일을 byte[]로 읽어와서 원본과 손상이 전혀 없는지를 확인
    assertThat(Files.readAllBytes(uploadDir.resolve(saved.get(0)))).isEqualTo(sent); // 파일명 전달
  }
  ...
  void 검증실패면_400() {
    // when
    ResponseEntity<String> response = multipartCreate("""
    ...
    assertThat((String) JsonPath.read(response.getBody(), "$.code")).isEqualTo("C001");
  ...
  void 생성_수정_삭제_생명주기() {
    ...
    ResponseEntity<String> updated = rest.exchange(one, HttpMethod.PUT, // 전송방식 직접 보낸다, exchange 메서드 - 만능, POST, GET... 보낼 수 있다
    ...
    ResponseEntity<Void> deleted = rest.exchange(one, HttpMethod.DELETE, null, Void.class); // delete는 리턴되는 것이 없고 요청과 함께 전달되는 데이터 없기 때문에 Void class
